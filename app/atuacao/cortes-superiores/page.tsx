...
import { ArrowLeft, MessageSquare, FileText, Landmark } from 'lucide-react';

// ============================================================================
// 1. MEUS DADOS (CORTES SUPERIORES)
// ============================================================================
const meusDados = {
  telefoneFormatado: "(84) 9970-2095",
  linkWhatsapp: "https://wa.me/558499702095?text=Olá, Dr. Adrianno. Necessito de atuação técnica nas Cortes Superiores (STJ/STF)."
};
...
export default function CortesSuperiores() {
      ...
      <section className="pt-40 md:pt-56 pb-24 px-6 lg:px-12 max-w-[1400px] mx-auto">
        <motion.div initial="hidden" animate="visible" variants={stagger} className="max-w-4xl">
          ...
          <motion.h1 variants={fadeInUp} className="text-6xl sm:text-7xl md:text-[8rem] font-serif font-bold leading-[0.9] tracking-tighter text-zinc-950 mb-12">
            Cortes <br /> <span className="italic font-medium text-zinc-400">Superiores.</span>
          </motion.h1>
          ...
          <motion.p variants={fadeInUp} className="text-xl md:text-2xl text-zinc-600 font-medium leading-relaxed max-w-2xl border-l-4 border-amber-600 pl-6">
            Quando as instâncias ordinárias falham, a atuação em Brasília (STJ e STF) torna-se o campo de batalha decisivo para a preservação das garantias constitucionais.
          </motion.p>
        </motion.div>
      </section>

      {/* TEXTOS E CAIXA DE AÇÃO CENTRALIZADA */}
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-12 gap-24">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-10%" }} variants={stagger} className="lg:col-span-7">
            <motion.h2 variants={fadeInUp} className="text-3xl md:text-5xl font-serif font-bold text-zinc-950 mb-10 leading-tight">
              Rigor acadêmico e elaboração de teses inovadoras.
            </motion.h2>
            <motion.p variants={fadeInUp} className="text-zinc-600 font-medium leading-relaxed text-lg mb-8">
              Interpor Recursos Especiais, Extraordinários e Habeas Corpus requer um domínio jurisprudencial cirúrgico. O Dr. Adrianno possui a expertise necessária para superar as barreiras de admissibilidade, muitas vezes intransponíveis para advogados não especializados.
            </motion.p>
            <motion.p variants={fadeInUp} className="text-zinc-600 font-medium leading-relaxed text-lg mb-12">
              Da elaboração minuciosa das peças recursais à entrega de memoriais e brilhantes sustentações orais perante os Ministros, a nossa missão é reverter injustiças e consolidar novos precedentes no ordenamento jurídico pátrio.
            </motion.p>
          </motion.div>

          {/* Lado Direito - Minha Caixa de Ação */}
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 1 }} className="lg:col-span-5">
            <div className="bg-zinc-50 border border-zinc-200 p-10 md:p-14 rounded-sm shadow-xl text-center flex flex-col items-center">
              
              {/* O Ícone de "Brasília/Tribunal" */}
              <div className="w-20 h-20 bg-zinc-950 rounded-full flex items-center justify-center text-amber-600 mb-8 shadow-2xl">
                <Landmark size={36} />
              </div>
              
              <h3 className="text-2xl font-serif font-bold text-zinc-950 mb-6">A Última Trincheira</h3>
              
              <p className="text-zinc-600 font-medium text-base leading-relaxed mb-10">
                A reversão de condenações e prisões ilegais no STJ e STF exige precisão. Avaliaremos a viabilidade recursal do seu processo com total franqueza técnica.
              </p>
              
              <a href={meusDados.linkWhatsapp} target="_blank" rel="noopener noreferrer" className="group inline-flex items-center justify-center space-x-4 py-5 bg-amber-600 text-white font-black uppercase tracking-widest text-[0.75rem] hover:bg-zinc-950 transition-all shadow-md w-full transform-gpu hover:-translate-y-1 active:scale-95">
